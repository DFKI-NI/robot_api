...
class RosWrapperInterface(ABC):
    ...
    def send_goal_and_wait(
        self, server_name: str, goal: T, timeout: float = 60.0
    ) -> Any:
        pass
    ...
    def wait_for_message(
        self, topic: str, message_type: Any, timeout: Optional[float] = None
    ) -> Any:
        pass
    ...
    def euler_from_quaternion(
        self, orientation: List[float], axes: str = "sxyz"
    ) -> Tuple[float, float, float]:
        return euler_from_quaternion(orientation, axes=axes)

    def quaternion_from_euler(
        self, roll: float, pitch: float, yaw: float, axes: str = "sxyz"
    ) -> List[float]:
    ...
    def _is_topic_with_suffix(self, namespace: str, topic: str, suffix: str) -> bool:
        for check_topic, check_message_type in self.get_topics(namespace):
            if check_topic == namespace + topic and check_message_type[0].endswith(
                suffix
            ):
                return True
        return False
    ...
    def _is_topic_of_type(self, namespace: str, topic: str, message_type: str) -> bool:
        for check_topic, check_message_type in self.get_topics(namespace):
            if (
                ...
                and check_message_type[0].split("/")[-1] == message_type
            ):
                return True
        return False
...
class Ros1Wrapper(RosWrapperInterface):
    ...
    def get_topics(self, namespace: str = "") -> List[Tuple[str, List[str]]]:
        return [
            ...
            for topic_name, topic_type in rospy.get_published_topics(namespace)
        ]
    ...
    def _has_action_result(self, server_name: str) -> bool:
        return self._is_topic_with_suffix(
            self._namespace, server_name + "/result", "Result"
        )
    ...
    def _connect_to_action_server(self, server_name: str, timeout: float = 0.0) -> bool:
        ...
        if server_name not in self._action_clients:
            ...
            if not action_client.wait_for_server(timeout=Duration(timeout)):
                rospy.logerr(
                    f"Timeout while trying to connect to server '{server_name}'.{' ROS is shutting down.' if rospy.is_shutdown() else ''}"
                )
                return False
    ...
    def send_goal_and_wait(
        ...
        return self._action_clients[server_name].send_goal_and_wait(
            goal, Duration(timeout)
        )

    def send_goal(self, server_name: str, goal: T, done_cb: Any) -> None:
        self._action_clients[server_name].send_goal(goal, done_cb)
    ...
    def lookup_transform(
        self, reference_frame: str, source_frame: str, time: int = 0
    ) -> Tuple[Tuple[float, float, float], Tuple[float, float, float, float]]:
    ...
    def create_open_gripper_goal(
        self, position: float = 0.1, max_effort: float = 100.0
    ) -> GripperCommandGoal:
    ...
    def create_close_gripper_goal(
        self, position: float = 0.0, max_effort: float = 50.0
    ) -> GripperCommandGoal:
        goal = GripperCommandGoal()
        goal.command.position = position
        goal.command.max_effort = max_effort
...
class Ros2Wrapper(RosWrapperInterface):
    ...
    def get_action_result(self, server_name: Optional[str] = None) -> Any:
        ...
        if server_name is None:
            ...
            if server_name is None:
                self.log(
                    "Cannot get result before connecting to any server.", level="error"
                )
                return None
        if server_name not in self._action_clients:
    ...
    def init_action_server(
        ...
        assert (
            server_specs
        ), "Error: You must init an action server with an item in server_specs."
        self._server_specs.update(server_specs)
    ...
    def _connect_to_action_server(self, server_name: str, timeout: float = 0.0) -> bool:
        ...
        if server_name not in self._action_clients:
            ...
            action_client = ActionClient(
                node, server_spec[0], self._namespace + server_name
            )
            if (
                not action_client.wait_for_server(timeout_sec=0.0)
                and len(server_spec) == 3
            ):
                self._execute(*server_spec[1:])
            if not action_client.wait_for_server(timeout_sec=timeout):
                self.log(
                    f"Timeout while trying to connect to server '{server_name}'.{' ROS is shutting down.' if not rclpy.ok() else ''}",
                    level="error",
                )
                return False
    ...
    def _has_action_result(self, server_name: str) -> bool:
        return self._is_topic_with_suffix(
            self._namespace, server_name + "/_action/get_result", "Result"
        )
    ...
    def send_goal_and_wait(
        ...
        rclpy.spin_until_future_complete(
            self._ros_node, send_goal_future, timeout_sec=timeout / 2.0
        )
        ...
        rclpy.spin_until_future_complete(
            self._ros_node, result_future, timeout_sec=timeout
        )
    ...
    def wait_for_message(self, topic, message_type, timeout=None):
        ...
        if not msg_received:
            raise TimeoutError(
                f"No message received on {topic} within {timeout} seconds."
            )
        return msg
